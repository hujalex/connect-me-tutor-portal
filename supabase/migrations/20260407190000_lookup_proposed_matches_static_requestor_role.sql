-- Extend lookup_proposed_matches_static with optional requestor role (student | tutor).
-- When set, only candidates whose JSON "role" is the opposite type are scored (mirrors queue direction).
-- Candidates with no "role" still participate (backward compatible).

DROP FUNCTION IF EXISTS public.lookup_proposed_matches_static(text[], jsonb, text[], integer);
...
COMMENT ON FUNCTION public.lookup_proposed_matches_static(text[], jsonb, text[], integer, text) IS
  'Static ranked matches. Optional p_requestor_role student|tutor: only candidates with opposite JSON "role" are kept; '
  'candidates without role are still included. Same Jaccard + rank_score as lookup_proposed_matches.';
